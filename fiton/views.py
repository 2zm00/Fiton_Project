...
def class_ticket_list(request,pk):
    classes=Class.objects.get(pk=pk)
    class_tickets=ClassTicket.objects.filter(class_type_id=classes.class_type.id)
    return render(request,'fiton/class_ticket_list.html',context={'class_tickets':class_tickets})
